...
then
    ...
    # 定义原始Cross-encoder路径和你的CASA Cross-encoder路径
    ORIGINAL_CROSS_ENCODER_PATH="./models/text2natsql_schema_item_classifier"
    CASA_CROSS_ENCODER_PATH="./models/casa_schema_item_classifier" # <--- 你的CASA模型保存路径
elif [ $1 = "large" ]
then
...
python predict_with_casa.py \ # <--- 调用你的预测脚本
    --batch_size 32 \
    --device $device \
    --model_path ${CASA_CROSS_ENCODER_PATH} \ # <--- 指定你的CASA模型路径
    --dataset_filepath "./data/preprocessed_data/preprocessed_test_natsql.json" \ # <--- 输入预处理数据
    --output_filepath "./data/preprocessed_data/casa_test_with_probs_natsql.json" \ # <--- 输出带CASA概率的文件
    --use_contents \
    --mode "test" \
    --add_fk_info
...
python text2sql_data_generator.py \
    --input_dataset_path "./data/preprocessed_data/casa_test_with_probs_natsql.json" \ # <--- 加载你的CASA概率文件
    --output_dataset_path "./data/preprocessed_data/resdsql_test_natsql_casa.json" \ # <--- 生成新的T5输入文件 (命名区分)
    --topk_table_num 4 \
    --topk_column_num 5 \
    --mode "test" \
    --use_contents \
...
python text2sql.py \
    ...
    --mode "eval" \
    --dev_filepath "./data/preprocessed_data/resdsql_test_natsql_casa.json" \ # <--- 加载新的T5输入文件
