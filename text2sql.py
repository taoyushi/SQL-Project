...
from self_correction.self_correction_module import self_correction_main
...
def parse_option():
    ...
    parser.add_argument('--enable_self_correction', action='store_true', help='是否启用自修正模块')
    parser.add_argument('--correction_config_path', type=str, default=None, help='自修正配置文件路径（如 configs/self_correction_config_optimized.yaml）')  #修改路径以调试代码
...
def _test(opt):
    ...
    for batch in tqdm(dev_dataloder):
        ...
        all_db_ids.extend(batch_db_ids)
        ...
        with torch.no_grad():
            ...
            if opt.target_type == "sql":
                ...
                raise ValueError()
    # ====== 自修正模块调用 ======
    if opt.enable_self_correction:
        print("[INFO] 启用自修正模块，开始修正 SQL ...")
        from self_correction.self_correction_module import self_correction_main
        # 支持输出详细日志
        detailed_output_path = None
        if opt.output.endswith('.sql'):
            detailed_output_path = opt.output.replace('.sql', '_detailed.json')
        predict_sqls = self_correction_main(
            predict_sqls=predict_sqls,
            db_ids=all_db_ids,
            questions=all_inputs,
            config_path=opt.correction_config_path,
